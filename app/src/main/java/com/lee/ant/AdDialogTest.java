...
public class AdDialogTest extends DialogFragment {
    private static final String TAG_DIALOG = "AdDialog";

    private ValueAnimator mAnimator;

    private static volatile AdDialogTest mInstance;

    @SuppressLint("ValidFragment")
    private AdDialogTest() {
    }

    public static AdDialogTest getInstance() {
        if (mInstance == null) {
            synchronized (AdDialogTest.class) {
                if (mInstance == null) {
                    mInstance = new AdDialogTest();
                }
            }
        }
        return mInstance;
