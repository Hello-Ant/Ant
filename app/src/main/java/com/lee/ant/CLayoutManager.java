...
public class CLayoutManager extends RecyclerView.LayoutManager {
    public static final String TAG = "CLayoutManager";
    /**
     * 堆叠方向在左
     */
    ...
    private CLayoutManager(Builder builder) {
...
// recycler.getScrapList().size());
    ...
    private int fillHorizontalLeft(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        mLastVisiPos = Math.min(mFirstVisiPos + maxLayerCount * 2, getItemCount() - 1);
...
// + mFirstVisiPos);
        ...
        //----------------3、开始布局-----------------
        final List<Integer> lazyLayout = new ArrayList<>();
        ...
        for (int i = mFirstVisiPos; i <= mLastVisiPos; i++) {
            ...
            } else if (i - mFirstVisiPos == maxLayerCount) {
                lazyLayout.add(i);
            } else if (i - mFirstVisiPos <= maxLayerCount * 2) {
                lazyLayout.add(i);
            } /*else {//属于普通区域
        ...
        final float centerStartX = startX + layerPadding;

        for (int i = lazyLayout.size() - 1; i >= 0; i--) {
            final int p = lazyLayout.get(i);
            Log.e(TAG, "fillHorizontalLeft: " + p);
            if (p - mFirstVisiPos == maxLayerCount) {
                View item;

                if (p == tempPosition && tempView != null) {
                    //如果初始化数据时已经取了一个临时view，可别浪费了！
                    item = tempView;
                } else {
                    item = recycler.getViewForPosition(p);
                }
                addView(item);
                measureChildWithMargins(item, 0, 0);
                ...
                if (trasitionListeners != null && !trasitionListeners.isEmpty()) {
                    for (TrasitionListener trasitionListener : trasitionListeners) {
                        trasitionListener.handleLayerView(this, item, p - mFirstVisiPos,
                                maxLayerCount, p, fraction, dx);
                    }
                }
                ...
                l = (int) centerStartX;
                t = getPaddingTop();
                r = (int) (centerStartX + getDecoratedMeasurementHorizontal(item));
                b = getPaddingTop() + getDecoratedMeasurementVertical(item);
                layoutDecoratedWithMargins(item, l, t, r, b);
            } else {
                View item;
                ...
                startX = centerStartX + (p - mFirstVisiPos) * layerPadding;

                if (trasitionListeners != null && !trasitionListeners.isEmpty()) {
                    for (TrasitionListener trasitionListener : trasitionListeners) {
                        trasitionListener.handleRightLayerView(this, item, p - mFirstVisiPos,
                                maxLayerCount, p, fraction, dx);
                    }
                }
...
// onceCompleteScrollLength) / (onceCompleteScrollLength * 1.0f));
    ...
    public static class Builder {
        ...
        public CLayoutManager build() {
            return new CLayoutManager(this);
        }
    }
    ...
    public interface TrasitionListener {
         ...
         */
        void handleLayerView(CLayoutManager focusLayoutManager, View view, int viewLayer,
                             int maxLayerCount, int position, float fraction, float offset);

        void handleRightLayerView(CLayoutManager focusLayoutManager, View view, int viewLayer,
                                  int maxLayerCount, int position, float fraction, float offset);
         ...
         * @param offset             当次滑动偏移量
         */
        void handleFocusingView(CLayoutManager focusLayoutManager, View view, int position,
         ...
         * @param offset             当次滑动偏移量
         */
        void handleNormalView(CLayoutManager focusLayoutManager, View view, int position,
                              float fraction, float offset);
...
//            log("layer =" + viewLayer + ";alpha = " + realAlpha + ";fraction = " + fraction);
        ...
        public void handleRightLayerView(CLayoutManager focusLayoutManager, View view, int viewLayer, int maxLayerCount, int position, float fraction, float offset) {
            ...
            float scaleDelta = maxScale - minScale; //总缩放差
            float currentLayerMaxScale =
                    minScale + scaleDelta * (maxLayerCount * 2 - viewLayer) / (maxLayerCount * 1.0f);
            float currentLayerMinScale = minScale + scaleDelta * (maxLayerCount * 2 - viewLayer + 1) / (maxLayerCount * 1.0f);
            float realScale =
                    currentLayerMaxScale - (currentLayerMaxScale - currentLayerMinScale) * realFraction;
