...
public class HahaPager extends ViewGroup implements Runnable {
    ...
    private static final float DEFAULT_MIDDLE_ALPHA = .7F;

    private static final float DEFAULT_BOTTOM_SCALE = .6F;
    private static final float DEFAULT_BOTTOM_ALPHA = .6F;

    private static final float DEFAULT_SMALL_SCALE = .4F;
    private static final float DEFAULT_SMALL_ALPHA = .5F;
    ...
    private float mTopScale, mMiddleScale, mBottomScale, mSmallScale;//缩放比例
    private float mTopAlpha, mMiddleAlpha, mBottomAlpha, mSmallAlpha;//不透明度
    ...
    public HahaPager(Context context) {
        this(context, null);
    }

    public HahaPager(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HahaPager(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mSmallScale = a.getFloat(R.styleable.LitePager_smallScale, DEFAULT_SMALL_SCALE);
        mSmallAlpha = a.getFloat(R.styleable.LitePager_smallAlpha, DEFAULT_SMALL_ALPHA);
    ...
    private void fixOverflow() {
        ...
        mSmallScale = fixOverflow(mSmallScale);
        mSmallAlpha = fixOverflow(mSmallAlpha);
    }
     ...
     */
    public HahaPager addViews(@NonNull @LayoutRes int... layouts) {
     ...
     * @param views 目标子View
     */
    public HahaPager addViews(@NonNull View... views) {
        for (View view : views) {
    ...
    private void updateChildOrder() {
        if (Math.abs(mOffsetPercent) > .5F) {
            if (!isReordered) {
                if (is7Child()) {
                    if (mOffsetPercent > 0) {
                        reOrder(0, 3, 1, 5, 2, 6, 4);
                    } else {
                        reOrder(2, 0, 4, 1, 6, 3, 5);
                    }
                } else if (is5Child()) {
                    if (mOffsetPercent > 0) {
                        reOrder(0, 3, 1, 4, 2);
    ...
    private List<View> mTempViewList = new ArrayList<>(7);
    ...
    private void updateFromAndTo() {
        if (Math.abs(mOffsetPercent) >= 1) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                Log.e("Ant", "updateFromAndTo: from: " + lp.from + " to: " + lp.to);
                lp.from = lp.to;
            }
            isReordered = false;
            mOffsetPercent %= 1;
            ...
            if (is7Child()) {
                switch (lp.from) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        lp.to = mOffsetPercent > 0 ? 1 : 5;
                        break;
                    case 4:
                        lp.to = mOffsetPercent > 0 ? 6 : 2;
                        break;
                    case 5:
                        lp.to = mOffsetPercent > 0 ? 3 : 6;
                        Log.e("Ant", "updateFromAndTo5: " + lp.to);
                        break;
                    case 6:
                        lp.to = mOffsetPercent > 0 ? 5 : 4;
                    default:
                        break;
                }
            } else if (is5Child()) {
    ...
    private boolean handleActionUp(float x, float y) {
        ...
        if (Math.abs(offsetX) < mTouchSlop && Math.abs(offsetY) < mTouchSlop) {
            ...
            if (hitView != null) {
                if (indexOfChild(hitView) == (is7Child() ? 6 : is5Child() ? 4 : 2)) {
    ...
    private int getBaselineByChild(View child) {
        int baseLine;
        if (is7Child()) {
            baseLine = isHorizontal() ? getHorizontalBaseLineBy7Child(child) : getVerticalBaseLineBy7Child(child);
            updateAlphaAndScaleBy7Child(child);
        } else if (is5Child()) {
    ...
    private int getHorizontalBaseLineBy7Child(View child) {
        return getBaseLineBy7Child(child, getWidth() / 8);
    }

    private int getVerticalBaseLineBy7Child(View child) {
        return getBaseLineBy7Child(child, getHeight() / 8);
    }

    private int getBaseLineBy7Child(View child, int itemDistance) {
        int baseLine = 0;
        int child0Line, child1Line, child2Line, child3Line, child4Line, child5Line, child6Line;
        child0Line = itemDistance;
        child2Line = itemDistance * 2;
        child4Line = itemDistance * 3;
        child6Line = itemDistance * 4;
        child5Line = itemDistance * 5;
        child3Line = itemDistance * 6;
        child1Line = itemDistance * 7;
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
            ...
            case 5:
                switch (lp.to) {
                    ...
                    case 6:
                        ...
                        Log.e("Ant", "getBaseLineBy7Child: " + baseLine + " offset: " + offset);
                        break;
                    default:
                        baseLine = child5Line;
                        break;
     ...
     * @param child 目标子View
     */
    private void updateAlphaAndScaleBy7Child(View child) {
        LayoutParams lp = (LayoutParams) child.getLayoutParams();
        switch (lp.from) {
            case 0:
                updateAlphaAndScaleBy7Child(child, lp, mOffsetPercent);
                break;
            case 1:
                updateAlphaAndScaleBy7Child(child, lp, -mOffsetPercent);
                break;
            case 2:
                updateAlphaAndScale7Three(lp, mOffsetPercent);
                break;
            case 3:
                ...
                break;
            case 4:
                updateAlphaAndScale7Two(lp, mOffsetPercent);
                break;
            case 5:
                updateAlphaAndScale7Two(lp, -mOffsetPercent);
                break;
    ...
    private void updateAlphaAndScaleBy7Child(View child, LayoutParams lp, float offsetPercent) {
        switch (lp.to) {
            case 0:
            case 1:
                setAsBottomBy5Child(child);
                lp.alpha = mSmallAlpha;
                lp.scale = mSmallScale;
                break;
            default:
                float alphaProgress;
                if (offsetPercent > .5F) {
                ...
                lp.alpha = mSmallAlpha + (mBottomAlpha - mSmallAlpha) * alphaProgress;
                lp.scale = mSmallScale + (mBottomScale - mSmallScale) * offsetPercent;
                break;
        }
    }

    private void updateAlphaAndScale7Three(LayoutParams lp, float offsetPercent) {
        float alphaProgress;
        ...
        switch (lp.to) {
            case 0:
            case 1:
                lp.alpha = mBottomAlpha + (mBottomAlpha - mSmallAlpha) * -alphaProgress;
                lp.scale = mBottomScale + (mBottomScale - mSmallScale) * offsetPercent;
                break;
            case 4:
            case 5:
    ...
    private void updateAlphaAndScale7Two(LayoutParams lp, float offsetPercent) {
        float alphaProgress;
        if (Math.abs(offsetPercent) > .5F) {
            alphaProgress = (Math.abs(offsetPercent) - .5F) * 2;
        } else {
...
//            throw new IllegalStateException("LitePager can only contain 5 child!");
    ...
    public void run() {
        if (mAutoScrollOrientation == SCROLL_ORIENTATION_LEFT) {
            setSelection(is7Child() ? 4 : is5Child() ? 2 : 0);
        } else {
            setSelection(is7Child() ? 5 : is5Child() ? 3 : 1);
        }
        if (mAutoScrollEnable) {
     ...
     * 设置是否开启自动轮播
     */
    public HahaPager setAutoScrollEnable(boolean enable) {
     ...
     */
    public HahaPager setAutoScrollInterval(long interval) {
     ...
     */
    public HahaPager setAutoScrollOrientation(@ScrollOrientation int orientation) {
     ...
     * @param adapter 适配器
     */
    public HahaPager setAdapter(Adapter adapter) {
        if (mAdapter != null) {
